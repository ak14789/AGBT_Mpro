...
def train_valid_split(dataset, train_rate: float = 0.8, process_fn=None):
    """
    将Dataset类划分为训练集Dataset和验证集Dataset
    ...
    if process_fn is not None:
        ...
        train_index_init = indexes[:int(len(indexes) * train_rate)]
        valid_index_init = indexes[int(len(indexes) * train_rate):]
